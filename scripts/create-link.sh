...
if [ $# -eq 0 ]; then
  # Pick source from anfra-common
  echo "Select source:"
  mapfile -t sources < <(find "$REPO_ROOT/plugins/anfra-common" -mindepth 2 -maxdepth 2 -type d | sed "s|$REPO_ROOT/||" | sort)
  select SOURCE_REL in "${sources[@]}"; do
    [ -n "$SOURCE_REL" ] && break
    echo "Invalid selection."
  done

  # Pick destination plugin (exclude anfra-common)
  echo ""
  echo "Select destination plugin:"
  mapfile -t plugins < <(find "$REPO_ROOT/plugins" -mindepth 1 -maxdepth 1 -type d -not -name 'anfra-common' | sed "s|$REPO_ROOT/||" | sort)
